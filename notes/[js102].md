...
# 模組化與 Library (這是 ES 5 用法)
> ES 6 的用法為 import 與 export，後面會提到。
## 引用別人的模組
``` js
...
1.  開啟命令提示字元，cd 到想要安裝 babel 的資料夾
2.  輸入 `npm init -y`，在 `node.js` 環境下建立一個 `package.json` 檔案
2.  輸入 `npm install --save-dev @babel/core @babel/cli @babel/preset-env @babel/node`
3.  輸入 `touch .babelrc`，建立一個隱藏檔
4.  開起 `.babelrc`，填入以下內容：
```
{
    "presets": [
...
```
然後即可使用 ` npx babel-node [JS 檔案]` 指令
範例：` npx babel-node main.js`

更多詳細使用方式可參考 [Babel 官網](https://babeljs.io/docs/en/babel-cli) 或 [[Webpack][ES6][Babel] 編譯#05. 安裝與設定Webpack, 利用babel loader編譯成ES5程式](https://www.youtube.com/watch?v=6-K7VHSAnXA)

## Jest
> 此內容參考自 [Jest | 讓 Jest 為你的 Code 做測試-基礎用法教學](https://medium.com/enjoy-life-enjoy-coding/%E8%AE%93-jest-%E7%82%BA%E4%BD%A0%E7%9A%84-code-%E5%81%9A%E5%96%AE%E5%85%83%E6%B8%AC%E8%A9%A6-%E5%9F%BA%E7%A4%8E%E7%94%A8%E6%B3%95%E6%95%99%E5%AD%B8-d898f11d9a23)。
### Jest 安裝懶人包
